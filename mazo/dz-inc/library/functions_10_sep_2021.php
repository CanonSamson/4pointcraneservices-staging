<?php	
					...
					<div class="clearfix">
						<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="comment-form"<?php echo esc_attr($html5) ? ' novalidate' : ''; ?>>
							...
							<?php if ( is_user_logged_in() ) : ?>
								...
								?>
							<?php else : ?>
								<?php echo wp_kses($args['comment_notes_before'], $allowed_html_tags); ?>
								<?php
								/**
							...
							?>
							<?php echo wp_kses($args['comment_notes_after'], $allowed_html_tags); ?>
								<p class="col-md-12 col-sm-12 col-xs-12 form-submit">
									<button id="<?php echo esc_attr( $args['id_submit'] ); ?>" type="submit" class="btn shadow-primary btn-primary">
	...
	function mazo_the_pagination($query = array(), $args = array(), $echo = 1)
	{
		...
		$pagination = str_replace("<ul class='page-numbers'", '<ul class="pagination text-center p-t20 mb-lg-0 mb-5"', paginate_links($args) );		
...
function mazo_get_website_logo($logo_key = 'header_logo') 
{
	global $mazo_option;
	extract($mazo_option);
...
add_action('mazo_get_logo', 'mazo_get_website_logo',2);
...
if($page_banner_setting == 'custom')
	...
	if($show_banner)
      ...
      <!-- Banner  -->
		...
		<div class="<?php echo esc_attr($banner_class); ?>" <?php if(!empty($banner_image) && empty($banner_hide)) { ?>style="background-image:url(<?php echo esc_url($banner_image); ?>) " <?php } ?>>
			<div class="container">
...
if($post_banner_setting == 'custom')
	...
	if($show_banner)
      ...
      <!-- Banner  -->
		...
		<div class="<?php echo esc_attr($banner_class); ?> <?php if(empty($banner_image)) { echo "bnr-no-img"; } ?> " <?php if(!empty($banner_image)) { ?>style="background-image:url(<?php echo esc_url($banner_image); ?>)" <?php } ?>>
			<div class="container">
