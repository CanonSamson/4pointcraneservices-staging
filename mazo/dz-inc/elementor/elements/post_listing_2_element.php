<?php
	...
	if($post_listing_2_element_posts_in_categories) {
	...
	if($post_listing_2_element_only_featured_posts == 'yes') 
	{ 		
	...
	if($post_listing_2_element_selected_sidebar == 'No_Sidebar' || !is_active_sidebar( $post_listing_2_element_selected_sidebar ) || !mazo_is_theme_sidebar_active())
	{
	...
	$mazo_query_result['posts_per_page'] = $post_listing_2_element_no_of_posts;	
	$mazo_query_result['current_page'] = $page_no;
	$mazo_query_result['side_bar'] = $post_listing_2_element_selected_sidebar;
	$mazo_query_result['title_text_limit'] = $post_listing_2_element_text_limit;
	$mazo_query_result['show_column'] = $post_listing_2_element_column;
	$mazo_query_result['blog_view_container'] = $blog_view_container;	
	$max_num_pages = $query->max_num_pages;	
	
	$btn_link = $btn_text = $anchor_attribute = '';
				
	if (!empty($post_listing_2_element_link_title))
	{
		$btn_link = !empty($post_listing_2_element_link)?$post_listing_2_element_link:'';
		$btn_text = !empty($post_listing_2_element_link_title)?$post_listing_2_element_link_title:'';		
...
?>
<!-- Blog -->
<section class="content-inner-1" id="DZPostListingElement2" <?php if(!empty($post_listing_2_element_bg_img['url'])){ ?> style="background-image:url(<?php echo esc_url($post_listing_2_element_bg_img['url']); ?>);background-position: bottom right;background-repeat: no-repeat;background-size: 40%;" <?php } ?>>
  <div class="container">
      ...
      <div class="<?php echo esc_attr($col_classes); ?>" >
		...
		<?php if(!empty($btn_text)) { ?>
			<div class="text-center">
				<a href="<?php echo esc_url($btn_link['url']); ?>" <?php echo esc_html($anchor_attribute); ?> class="btn btn-outline-primary">
      ...
      <?php
				...
				{	
					...
					<a href="javascript:void(0);" class="btn shadow-primary btn-primary btn-rounded btn-ov-secondary loadmore-btn dz-load-more " id="<?php echo esc_attr($blog_view_btn); ?>"   
						...
						data-show-column="<?php echo esc_js($post_listing_2_element_column);?>"
