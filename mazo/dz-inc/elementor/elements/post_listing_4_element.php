<?php
	...
	$query_args = array(	
		'post_type' 		=> 'post',
		'post_status' 		=> 'publish',
		'posts_per_page'   	=> $post_listing_4_element_no_of_posts['size'] ,		
	...
	if($post_listing_4_element_posts_in_categories) {
	...
	if($post_listing_4_element_only_featured_posts == 'yes') 
	{ 		
	...
	if($post_listing_4_element_selected_sidebar == 'No_Sidebar' || !is_active_sidebar( $post_listing_4_element_selected_sidebar ) || !mazo_is_theme_sidebar_active())
	{
	...
	$blog_view_container = '';
	if($post_listing_4_element_pagination_style == 'load_more')
	{
	...
	$mazo_query_result['posts_per_page'] = $post_listing_4_element_no_of_posts['size'];	
	$mazo_query_result['current_page'] = $page_no;
	$mazo_query_result['side_bar'] = $post_listing_4_element_selected_sidebar;
	$mazo_query_result['title_text_limit'] = $post_listing_4_element_text_limit;
...
?>
<!-- Blog -->
<section class="content-inner" id="DZPostListingElement4">
  <div class="container">
    <?php if(!empty($post_listing_4_element_title)){ ?>
		<div class="section-head style-1 text-center">
			...
			<?php if(!empty($post_listing_4_element_description)){ ?>
				<p>
					<?php echo wp_kses($post_listing_4_element_description,'string'); ?>
				</p>
			<?php } ?>
		</div>
	<?php } ?>
    <div class="row">
      <?php 
        if ($post_listing_4_element_sidebar_layout == 'left' && 
            is_active_sidebar( $post_listing_4_element_selected_sidebar ) && 
            mazo_is_theme_sidebar_active() ){ 
        ?>
        <div class="col-xl-4 col-lg-4 dz-order-1">
			<aside class="side-bar sticky-top left">
            <?php dynamic_sidebar( $post_listing_4_element_selected_sidebar ); ?>
          </aside>
        </div>
      <?php } ?>
      <!-- Side bar END -->
      ...
      <div class="<?php echo esc_attr($col_classes); ?>" >
        <div <?php if(!empty($blog_view_container)){ ?> id="<?php echo esc_attr($blog_view_container) ?>" <?php } ?> class="row blog-area1 spno">
        <?php get_template_part('dz-inc/elementor/ajax/post_listing_4_ajax'); ?>
        </div>
        <!-- Side bar END -->
      <?php
				if($post_listing_4_element_pagination_style == 'load_more')
				{	
					...
					<a href="javascript:void(0);" class="btn shadow-primary btn-primary btn-rounded btn-ov-secondary loadmore-btn dz-load-more " id="<?php echo esc_attr($blog_view_btn); ?>"   
						...
						data-posts-per-page="<?php echo esc_js($post_listing_4_element_no_of_posts['size']);?>"
						data-image-preference="<?php echo esc_js($post_listing_4_element_image_preference); ?>"
						data-only-featured-post="<?php echo esc_js($post_listing_4_element_only_featured_posts)?>" 
						data-post-order="<?php echo esc_js($post_listing_4_element_order)?>"	
						data-post-order-by="<?php echo esc_js($post_listing_4_element_orderby)?>"	
						data-posts-in-categories="<?php echo esc_js($post_listing_4_element_posts_in_categories);?>"
						data-side-bar="<?php echo esc_js($post_listing_4_element_selected_sidebar);?>"
						data-title-text-limit="<?php echo esc_js($post_listing_4_element_text_limit);?>" 
						>
      ...
      <?php 
        if ($post_listing_4_element_sidebar_layout == 'right' && 
            is_active_sidebar( $post_listing_4_element_selected_sidebar ) && 
            mazo_is_theme_sidebar_active() ){ 
        ?>
        <div class="col-xl-4 col-lg-4 dz-order-1">
